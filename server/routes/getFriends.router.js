...
router.get('/', rejectUnauthenticated, (req, res) => {
  ...
  const sqlText = `(select username, friends.approved, friends.user_id_two as friend_ID from "users" 
                        join friends on friends.user_id_two = "users".id 
                        where friends.user_id_one = $1)
                        union
                        (select username, friends.approved, friends.user_id_one as friend_ID from "users" 
                        join friends on friends.user_id_one = "users".id 
